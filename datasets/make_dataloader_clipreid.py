...
from .bases import ImageDataset,ImageDatasetCluster
from timm.data.random_erasing import RandomErasing
from .sampler import RandomIdentitySampler,RandomIdentityClusterSampler
...
from .muf_keti import MUF_KETI
from .msmt17clustered import MSMT17CLUSTERED
__factory = {
    'market1501': Market1501,
    'dukemtmc': DukeMTMCreID,
    'msmt17': MSMT17,
    'msmt17clustered': MSMT17CLUSTERED,
...
def train_clustered_collate_fn(batch):
    """
    # collate_fn for clustered scene
    """
    imgs, pids, camids, viewids , _,clusters = zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)
    viewids = torch.tensor(viewids, dtype=torch.int64)
    camids = torch.tensor(camids, dtype=torch.int64)
    clusters = torch.tensor(clusters, dtype=torch.int64)
    return torch.stack(imgs, dim=0), pids, camids, viewids,clusters
...
def make_dataloader(cfg):
    ...
    if cfg.DATASETS.CLUSTER:
        train_set = ImageDatasetCluster(dataset.train, train_transforms)
        train_set_normal = ImageDatasetCluster(dataset.train, val_transforms)
    else:
        train_set = ImageDataset(dataset.train, train_transforms)
        train_set_normal = ImageDataset(dataset.train, val_transforms)
        ...
        if cfg.MODEL.DIST_TRAIN:
            ...
            if cfg.DATASETS.CLUSTER:
                train_loader_stage2 = DataLoader(
                    ...
                    sampler=RandomIdentityClusterSampler(dataset.train, cfg.SOLVER.STAGE2.IMS_PER_BATCH, cfg.DATALOADER.NUM_INSTANCE),
                    num_workers=num_workers, collate_fn=train_clustered_collate_fn
                )
            else:
                train_loader_stage2 = DataLoader(
                    train_set, batch_size=cfg.SOLVER.STAGE2.IMS_PER_BATCH,
    ...
    if cfg.DATASETS.CLUSTER:
        train_loader_stage1 = DataLoader(
            ...
            collate_fn=train_clustered_collate_fn
        )
    else:
        train_loader_stage1 = DataLoader(
            train_set_normal, batch_size=cfg.SOLVER.STAGE1.IMS_PER_BATCH, shuffle=True, num_workers=num_workers,
