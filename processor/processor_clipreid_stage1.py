...
def do_train_stage1_cluster(cfg,
                    model,
                    train_loader_stage1,
                    optimizer,
                    scheduler,
    ...
    with torch.no_grad():
        for n_iter, (img, vid, target_cam, target_view,cluster) in enumerate(train_loader_stage1):
            ...
            with amp.autocast(enabled=True):
                image_feature = model(img, target, get_image=True)
                for i,clust, img_feat in zip(target,clusters, image_feature):
                    labels.append(i)
                    cluster_list.append(clust)
                    image_features.append(img_feat.cpu())
        labels_list = torch.stack(labels, dim=0).cuda()  # N
        clusters_list = torch.stack(cluster_list, dim=0).cuda()
        image_features_list = torch.stack(image_features, dim=0).cuda()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i in range(i_ter + 1):
            ...
            target = labels_list[b_list]
            image_features = image_features_list[b_list]
            cluster = clusters_list[b_list]
            with amp.autocast(enabled=True):
                text_features = model.forward_clustered(label=target, get_text=True,cluster=cluster)
            loss_i2t = xent(image_features, text_features, target, target)
            loss_t2i = xent(text_features, image_features, target, target)
